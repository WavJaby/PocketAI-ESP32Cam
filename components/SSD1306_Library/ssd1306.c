...
static uint8_t ssd1306_rotate_byte(uint8_t ch1);
...
inline void ssd1306_init(SSD1306_t *dev, int width, int height) {
#ifdef CONFIG_SPI_INTERFACE
...
inline int ssd1306_get_width(SSD1306_t *dev) {
    return dev->_width;
}

inline int ssd1306_get_height(SSD1306_t *dev) {
    return dev->_height;
}

inline int ssd1306_get_pages(SSD1306_t *dev) {
    return dev->_pages;
}

inline void ssd1306_show_buffer(SSD1306_t *dev) {
#ifdef CONFIG_SPI_INTERFACE
...
inline void ssd1306_show_buffer_page(SSD1306_t *dev, int page, int seg) {
#ifdef CONFIG_SPI_INTERFACE
    spi_display_image(dev, page, seg, dev->_page[page]._segs, dev->_width);
#else
    i2c_display_image(dev, page, seg, dev->_page[page]._segs, dev->_width);
#endif
}
...
inline void ssd1306_set_page(SSD1306_t *dev, int page, uint8_t *buffer) {
    memcpy(&dev->_page[page]._segs, buffer, 128);
}

inline void ssd1306_get_page(SSD1306_t *dev, int page, uint8_t *buffer) {
    memcpy(buffer, &dev->_page[page]._segs, 128);
}

inline void ssd1306_display_image(SSD1306_t *dev, int page, int seg, uint8_t *images, int width) {
#ifdef CONFIG_SPI_INTERFACE
...
void ssd1306_display_text(SSD1306_t *dev, int offset, int page, char *text, bool invert) {
    if (page >= dev->_pages) return;

    uint8_t image[8];
    for (int i = offset; i < 16 && *text; i++, text++) {
        memcpy(image, font8x8_basic_tr[(uint8_t)*text], 8);
        if (invert) ssd1306_invert(image, 8);
        if (dev->_flip) ssd1306_flip(image, 8);
        ssd1306_display_image(dev, page, i << 3, image, 8);
    }
}
...
#ifdef CONFIG_SPI_INTERFACE
    for (int page = 0; page < dev->_pages; page++) {
        memset(dev->_page[page]._segs, invert ? 0xFF : 0x00, sizeof(dev->_page[page]._segs));
        spi_display_image(dev, page, 0, dev->_page[page]._segs, dev->_width);
    }
#else
    for (int page = 0; page < dev->_pages; page++) {
...
void ssd1306_scroll_text(SSD1306_t *dev, char *text, bool invert) {
    ...
    ssd1306_display_text(dev, 0, srcIndex, text, invert);
}
...
// Rotate 8-bit data
// 0x12-->0x48
static inline uint8_t ssd1306_rotate_byte(uint8_t ch1) {
    return ((ch1 >> 7) & 0b00000001) | ((ch1 >> 5) & 0b00000010) | ((ch1 >> 3) & 0b00000100) | ((ch1 >> 1) & 0b00001000) |
           ((ch1 << 1) & 0b00010000) | ((ch1 << 3) & 0b00100000) | ((ch1 << 5) & 0b01000000) | ((ch1 << 7) & 0b10000000);
}
